...
9. **解析数据** → 从 `.title a` 提取小区名（格式 "小区名 室型 朝向"，取第一段）；从 `.totalPrice span` 提取总价；从 `.unitPrice span` 提取单价；记录爬取时间
...
| 小区搜索返回全城数据 | 贝壳网小区名不存在或名称不同，建议换一个小区名 |
| 选择器失效（找不到元素） | 贝壳网页面结构可能更新，需检查并更新 CSS 选择器 |
...
| 数据字段 | CSS 选择器 | 说明 |
|---------|-----------|------|
| 房源列表项 | `.sellListContent li.clear` | 每个房源卡片 |
| 小区名称 | `.title a` | 文本格式 "小区名 室型 朝向"，取空格前第一段 |
| 总价 | `.totalPrice span` | 单位万元 |
| 单价 | `.unitPrice span` | 元/平米 |

> ⚠️ 如遇 `no such element` 错误，首先检查选择器是否已失效。
