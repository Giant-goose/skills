...
**关键规则：图片和文字必须分开发送，否则 WeChat 会重排序（文字先到，图片后到）。**

### 第 1-5 条房源（带图）
每条房源发送 **2 条独立消息**：
1. 消息1 = 图片（MEDIA:path.jpg，单独发送）
2. 消息2 = 文字（序号 + 小区 + 总价 + 单价）

```
**[1]** 中海右岸 | 125万 | 17,292元/平
```

### 第 6-30 条房源（纯文字列表）
单条消息，包含所有序号 + 信息，末尾附均价 + 询问：

```
[6] 中海右岸 3室 | 127万 | 16,125元/平
[7] 中海右岸 3室 | 128万 | 16,252元/平
...
[30] 中海右岸 3室 | 139万 | 15,543元/平

你对哪个房源感兴趣？回复序号即可获取链接
```

### 用户回复序号后
读取 Excel 第 N 行第 5 列（链接列），发送 URL。

### 均价说明
均价不单独一行显示，而是在列表末尾注明（如果有30条数据，均价 = 30条单价之和 / 30）。
