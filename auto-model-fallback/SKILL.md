...
**模型名称必须与 MiniMax 官方一致，大小写敏感！**

| 实际名称 | 错误写法 |
|---------|---------|
| `MiniMax-M2.7` | M2.7 |
| `MiniMax-M2.5-HighSpeed` | M2.5-Highspeed / M2.5highspeed / M2.5-highspeed |
| `MiniMax-M2.5` | M2.5 |

**验证方法**：API返回的 `model` 字段会显示实际识别的名称，名称不对会静默回落到默认模型。
...
- 如果所有模型都失败，会返回明确错误信息
- **模型名大小写必须完全匹配**，不正确的名称会导致静默回落到默认模型
